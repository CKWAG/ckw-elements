...
> Status: Token pipeline complete, Storybook active, Components package active
...
│       └── src/
│           ├── docs.css             CKW-branded docs stylesheet
│           ├── data/
...
| `dist/tokens.d.ts` | No        | Yes       | Yes              |
| `dist/tokens.json` | No        | Yes       | Static docs only |
...
Storybook also publishes generated token assets as static files for agents and
tooling:

```
packages/tokens/dist/
       │
       └─── staticDirs ─────▶ /tokens/tokens.css
                              /tokens/tokens.js
                              /tokens/tokens.d.ts
                              /tokens/tokens.json
```
...
### CKW Documentation Chrome

Storybook documentation pages use CKW design tokens for page surfaces, tables,
badges, navigation, and live token previews. `src/docs.css` is the central
stylesheet for this docs chrome. Legacy `--specs-*` variables and title classes
still exist in the stylesheet for compatibility, but new documentation should
use the CKW docs classes and semantic token references already present in the
Storybook pages.
...
Step 4    Run in terminal:
          ...
          └── build-tokens.mjs      → regenerates dist/tokens.css + dist/tokens.js + types
          ↓
...
- `dist/tokens.d.ts` — regenerated TypeScript declarations
- `dist/tokens.json` — copied DTCG JSON for static docs
...
| `dist/tokens.d.ts`            | TypeScript declarations for token JS          | No        | Yes       |
| `dist/tokens.json`            | DTCG JSON copy for static docs                | No        | Yes       |
...
| `public/fonts/Roboto-latin.woff2`                | Roboto variable font (legacy docs fallback)    |
| `public/fonts/Roboto-latin-ext.woff2`            | Roboto latin-ext subset                        |
| `public/fonts/JetBrainsMono-500-latin.woff2`     | JetBrains Mono code font                       |
...
| `src/docs.css`                                   | CKW-branded docs stylesheet                    |
